...
function _checkIfReimport ( assetdb, fspath, cb ) {
    var uuid = assetdb._uuid2path[fspath];
    ...
    // if we don't find the library file, check if this is an asset that needs to import
    var libpath = assetdb._uuid2libpath(uuid);
    if ( !Fs.existsSync(libpath) ) {
        if ( cb ) cb ( null, true );
        return;
    }

    // if library file exists, check the rawdata's mtime
    var mtimeInfo = this._uuid2mtime[uuid];
    if ( mtimeInfo ) {
...
function _refresh ( assetdb, fspath, cb ) {
    Async.waterfall([
        ...
        function ( paths, next ) {
            Async.each( paths, function ( path, done ) {
                _importAsset( assetdb, path, function ( err ) {
                    if ( err ) {
